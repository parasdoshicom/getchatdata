...
Claude's plugin hooks can count direct or agent-selected ChatData skills and the end of that turn. The optional ChatData status line appends estimated hours and value while preserving an existing status line. Codex and Cursor use the same `scripts/telemetry.py` counter through the selected skill; their totals appear in the personal dashboard and the local `status` command.
