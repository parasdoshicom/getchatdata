...
All 16 skills remain free; AI client fees can apply. The official ChatData workflow requires usage to be linked to the user's verified dashboard email. If this Claude installation is not linked, say that ChatData analysis will wait, then direct the user to `https://getchatdata.com/dashboard` to create and run the email-linked Claude Code command. Do not describe linking as optional. Setup, this status command, updates, footer recovery, and the bundled offline doctor remain available for repair. If the report says the time baseline is missing, explain that the user sets their usual minutes per workflow and that ChatData starts at $125/hour unless they change it. If there are queued events, show the exact flush command from the report but do not run it without a separate request. If no data was supplied and usage is linked, offer the bundled mix-shift example as the first task. Show the installed version instead of inventing the latest release.
