...
- **Website visits.** Cloudflare serves `getchatdata.com`. It may process ordinary request, performance, and security information needed to deliver and protect the site. The production website also loads a Cloudflare analytics and performance beacon. Those requests describe website visits, not calculations run through the local plugin. The website can also send application error events and the technical context configured for those events to PostHog. The plugin does not receive those website logs or error events.
